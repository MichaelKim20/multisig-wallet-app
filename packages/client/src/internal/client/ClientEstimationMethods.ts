...
export class ClientEstimationMethods extends ClientCore implements IClientEstimationMethods {
    ...
    public async create(
        ...
        const gasEstimation = await factoryInstance.estimateGas.create(name, description, owners, required);
        return this.web3.getApproximateGasFee(gasEstimation.toBigInt());
    }

    public async changeName(wallet: string, name: string): Promise<GasFeeEstimation> {
        const signer = this.web3.getConnectedSigner();
        if (!signer) {
            throw new NoSignerError();
        } else if (!signer.provider) {
        ...
        const gasEstimation = await factoryInstance.estimateGas.changeName(wallet, name);
        return this.web3.getApproximateGasFee(gasEstimation.toBigInt());
    }

    public async changeDescription(wallet: string, description: string): Promise<GasFeeEstimation> {
        const signer = this.web3.getConnectedSigner();
        if (!signer) {
            throw new NoSignerError();
        } else if (!signer.provider) {
        ...
        const gasEstimation = await factoryInstance.estimateGas.changeDescription(wallet, description);
        return this.web3.getApproximateGasFee(gasEstimation.toBigInt());
    }
